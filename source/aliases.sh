...
function ytdc() {
	...
	if [ "$dl" == "" ]; then
		echo "Error: No version of 'youtube-dl' or 'yt-dlp' found. Exiting..."
		return 1
	fi
...
alias srlist=srl
alias srs="snapraid scrub -v"
alias srscrub="snapraid scrub -v -p 100 -o 0"
...
function smarttest {
	...
	if [ "$1" == "--cancel" ]; then
		for each in $(ls /dev/sd?); do
			echo "Cancelling smart tests for disk '$each'..."
